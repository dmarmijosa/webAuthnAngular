...
export class WebAuthnService {
  ...
  webAuthnSignup(user: User): Promise<CredentialType> {
    ...
    const publicKeyCredentialCreationOptions: PublicKeyCredentialCreationOptions =
      {
        ...
        user: {
          ...
          id: Uint8Array.from(user.id as string, (c) => c.charCodeAt(0)),
          name: user.email,
    ...
    return navigator.credentials
      ...
      .then((credential) => {
        ...
        return credential as CredentialType; // Asegúrate de que esto corresponda a tu tipo CredentialType
      });
  }
  ...
  webAuthnSignin(user: User): Promise<CredentialType> {
    ...
    return navigator.credentials
      .get({ publicKey: credentialRequestOptions })
      .then((credential) => {
        if (!credential) {
          throw new Error('No se pudo obtener la credencial');
        }
        return credential as CredentialType; // Asume que tu transformación a CredentialType es válida
      });
  }
}
